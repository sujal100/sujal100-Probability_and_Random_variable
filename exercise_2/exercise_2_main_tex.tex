...
Consider,\\
$W_1 = $First remove ball is White.\\
$R_2 = $Second removed ball is Red.\\
Since, $W_1\&R_2$ are independent events.\\
So,
\begin{align}
Pr(W_1R_2) = Pr(W_1).Pr(R_2)
\end{align} 
And, required probability is 
\begin{align}
Pr(R_2/W_1) = \frac{Pr(W_1R_2)}{Pr(W_1)} = \frac{Pr(W_1).Pr(R_2)}{Pr(W_1)} = Pr(R_2)
\end{align}
We have 6 balls, one ball can be choose in $\comb{6}{1}$ ways, Since there are three red balls so probability that the second ball is red is $Pr(R_2)$
\begin{enumerate}
